...
export async function setupIDE(projectPath, ideTools) {
  // ideTools is now an array, so loop through and set up each
  for (const ide of ideTools) {
    if (ide === 'cursor') {
      await setupCursor(projectPath);
    } else if (ide === 'vscode') {
      await setupVSCode(projectPath);
    } else if (ide === 'claude-code') {
      await setupClaudeCode(projectPath);
    }
  }
}
...
async function setupCursor(projectPath) {
  ...
  if (!(await fs.pathExists(sourceCursor))) {
    console.warn('⚠️  Could not find Cursor config - skipping Cursor setup');
    return;
  }

  // Copy .cursor/ to user's project
  await fs.copy(sourceCursor, targetCursor, {
...
async function setupVSCode(projectPath) {
  ...
  if (!(await fs.pathExists(sourceGithub))) {
    console.warn('⚠️  Could not find VS Code config - skipping VS Code setup');
    return;
  }

  // Copy .github/ to user's project
  await fs.copy(sourceGithub, targetGithub, {
    overwrite: false,
    errorOnExist: false,
  });

  console.log('✅ VS Code config installed (.github/copilot-instructions.md)');
  console.log('   GitHub Copilot will now use FWD PRO experts!');
}
...
async function setupClaudeCode(projectPath) {
  ...
  if (!(await fs.pathExists(sourceClaude))) {
    console.warn('⚠️  Could not find Claude Code config - skipping Claude Code setup');
    return;
  }

  // Copy .claude/ to user's project
  await fs.copy(sourceClaude, targetClaude, {
    overwrite: false,
    errorOnExist: false,
  });

  console.log('✅ Claude Code rules installed (.claude/rules/)');
  console.log('   You can now use /denny, /ada, /lyna, etc. in Claude Code!');
}
