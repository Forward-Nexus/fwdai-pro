...
async function detectPythonStack(projectPath) {
  ...
  try {
    ...
    if (requirements.includes('postgresql') || requirements.includes('psycopg'))
...
async function findContextFiles(projectPath) {
  ...
  try {
    ...
    const contextPatterns = [/guide/i, /context/i, /brief/i, /overview/i, /about/i];
...
function extractMission(content) {
  ...
  const audienceMatch = content.match(
    /(?:target audience|for whom|who (?:is )?this (?:is )?for)[:\s-]+([^\n]+)/i
  );
  if (audienceMatch) {
...
function extractFounderStory(content) {
  ...
  const workingStyleMatch = content.match(
    /(?:communication|working\s+style|how\s+(?:I|we)\s+work)[:\s-]+([\s\S]{0,300})/i
  );
