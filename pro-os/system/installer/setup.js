...
export async function setupProject(answers) {
  ...
  try {
    ...
    const projectKB = await generateProjectKB(aboutYou, aboutProject, aboutProject.scanFindings);
    ...
    // Generate mission.md (use targetAudience from marketing if available, otherwise from mission section)
    const missionAnswers = {
      targetAudience:
        aboutProject.targetAudience ||
        (aboutProject.targetAudienceOther ? `Other: ${aboutProject.targetAudienceOther}` : null),
  ...
  try {
    await setupIDE(projectPath, aboutProject.ideTools);
