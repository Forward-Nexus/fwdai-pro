  ...
  const answers = await inquirer.prompt([
    ...
    },
    {
      ...
      choices: [
        ...
        {
          name: "AI-assisted builder (I use AI to code and design systems, but can't code myself)",
          value: 'ai-assisted',
        },
        {
          name: 'Semi-technical (I understand concepts, some coding experience)',
          value: 'semi-technical',
        },
        { name: 'Technical (I code regularly without AI)', value: 'technical' },
        {
          name: 'Very technical (I design systems and write complex code)',
          value: 'very-technical',
        },
    ...
    },
    {
      ...
      validate: input =>
        input.length > 0 ? true : 'Please select at least one communication style',
    },
    {
      type: 'input',
      name: 'commStyleOther',
...
async function askAboutProject() {
  ...
  if (projectType === 'existing') {
    ...
    if (scanFindings.projectName || scanFindings.description || scanFindings.techStack.length > 0) {
      ...
      if (scanFindings.projectName)
        console.log(chalk.gray(`  - Project: ${scanFindings.projectName}`));
      if (scanFindings.description)
        console.log(chalk.gray(`  - Description: ${scanFindings.description.substring(0, 60)}...`));
      if (scanFindings.techStack.length > 0)
        console.log(chalk.gray(`  - Tech stack: ${scanFindings.techStack.join(', ')}`));
  ...
  // Q10: AI tools (multiple selection)
  const { ideTools } = await inquirer.prompt([
    {
      type: 'checkbox',
      name: 'ideTools',
      message: 'Which AI coding tool(s) are you using? (select all that apply)',
      choices: [
        { name: 'Cursor', value: 'cursor' },
        { name: 'VS Code (with GitHub Copilot)', value: 'vscode' },
  ...
  if (workTypes.includes('building')) {
    ...
    const buildingAnswers = await inquirer.prompt([
      ...
      },
      {
        ...
        transformer: input =>
          chalk.gray('Examples: "React Native, Node.js, PostgreSQL"\n> ') + input,
      },
      {
        type: 'list',
        name: 'codeLocation',
    ...
    const legalAnswers = await inquirer.prompt([
      {
        ...
        choices: [
          ...
          {
            name: 'Business formation (Incorporation, Founder/Operating agreements)',
            value: 'formation',
          },
  ...
  const { goal } = await inquirer.prompt([
    {
      ...
      transformer: input =>
        chalk.gray(
          'Examples: Launch MVP by Q4, Raise seed round by Q2, Get to 1,000 users, Build and ship it\n> '
        ) + input,
  ...
  const { constraints } = await inquirer.prompt([
    {
      ...
      default: '',
      transformer: input =>
        chalk.gray('Examples: Need HIPAA compliance, Budget is tight, Deadline: Dec 31st\n> ') +
        input,
    },
  ]);
  ...
  if (wantsDomainExpert) {
    domainExpert = await inquirer.prompt([
      ...
      },
      {
        ...
        transformer: input =>
          chalk.gray(
            'Examples: Addiction recovery, Restaurant operations, Federal compliance\n> '
          ) + input,
    ...
    missionAnswers = await inquirer.prompt([
      {
        ...
        default: '',
        transformer: input =>
          chalk.gray('Example: "They can\'t track their goals effectively"\n> ') + input,
      },
      {
        type: 'input',
        name: 'yourWhy',
        ...
        default: '',
        transformer: input =>
          chalk.gray('This helps experts understand your passion and motivation\n> ') + input,
      },
    ]);
  }
    ...
    peopleInfo = await inquirer.prompt([
      {
        ...
        default: '',
        transformer: input =>
          chalk.gray('Example: "Sarah (marketing mentor), John (technical advisor)"\n> ') + input,
      },
      {
        type: 'input',
        name: 'investors',
        ...
        default: '',
        transformer: input =>
          chalk.gray('Example: "Angel investor Mike, Friends & family"\n> ') + input,
      },
      {
        type: 'input',
        name: 'keyConnections',
        ...
        default: '',
        transformer: input =>
          chalk.gray('Example: "Early customer Lisa, Industry contact Tom"\n> ') + input,
      },
    ]);
  }
