...
export async function updateInstallation() {
  ...
  try {
    ...
    spinner.succeed(chalk.green('✓ Expert customizations backed up'));

    // Step 3: Surgical update - only overwrite files we ship, preserve everything else
    spinner = ora('Updating system files...').start();

    // Surgical approach: Copy only what we ship, preserve user's custom additions
    const updatePaths = [
      'pro-os/commands',
      'pro-os/system',
      'pro-os/user-docs'
    ...
    // Handle experts specially - only update CORE experts, preserve custom ones
    const coreExperts = [
      'genna-architect.md',
    ...
    const sourceExpertsDir = path.join(FWD_PRO_SOURCE, 'experts');
    const targetExpertsDir = path.join(fwdproDir, 'pro-os', 'experts');
    
    for (const expertFile of coreExperts) {
      const sourcePath = path.join(sourceExpertsDir, expertFile);
      const targetPath = path.join(targetExpertsDir, expertFile);
    ...
    // Step 3.5: Three-way merge for CORE expert files only
    spinner = ora('Merging expert customizations...').start();
    
    const conflictedExperts = [];
    
    // Only merge core experts - custom domain experts are already preserved
    if (await fs.pathExists(expertsBackupDir)) {
      for (const expertFile of coreExperts) {
        ...
        try {
          ...
          console.log(chalk.yellow(`⚠️  Could not merge ${expertFile}: ${error.message}`));
        }
      }
    }
    
    spinner.succeed(chalk.green('✓ Expert files merged'));

    // Step 4: Clean up known deprecated files
    spinner = ora('Cleaning up deprecated files...').start();
    ...
    // Remove specific old files that were renamed/consolidated
    const deprecatedFiles = [
      'pro-os/commands/newthread.md',  // Removed in 2.0.0
      'pro-os/system/standards/workflow-standards.md',  // Consolidated in 2.0.0
      'pro-os/system/standards/roundtable-standards.md'  // Consolidated in 2.0.0
    ];
    
    for (const deprecatedFile of deprecatedFiles) {
      const filePath = path.join(fwdproDir, deprecatedFile);
    ...
    if (conflictedExperts.length > 0) {
      ...
      console.log(chalk.yellow('⚠️  Your AI experts may not work correctly until conflicts are resolved!\n'));
    }
